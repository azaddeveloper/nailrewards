...
import {
  ...
  StoreRatingDetails,
  TransactionHistory,
} from "src/app/models/customer-detail.model";
...
export class CustomerDetailsComponent implements OnInit, OnDestroy {
  ...
  getUserDetails() {
    ...
    let s1 = this.apiService.customerDetail(body).subscribe(
      (result) => {
        ...
        this.rating = parseInt(this.ratingDetails.rating);
  ...
  get f1() {
    return this.redeemRewardForm.controls;
  }
  get f2() {
    return this.transactionHistoryForm.controls;
  }
  ngOnInit() {
    ...
    this.transactionHistoryForm = this.formBuilder.group({
      spend_amount: ["", [Validators.required, Validators.pattern(/^\d*\.?\d{0,2}$/)]],
      remark: ["", [Validators.required]],
    });
  }
  ...
  openRedeemDialog(reward) {
    const modalRef = this.modalService.open(RedeemRewardDialogComponent);
